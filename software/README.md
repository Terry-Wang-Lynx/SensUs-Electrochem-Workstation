...
界面的快速预设按“两种电极 × 氧化/还原两步”排成四个。⚠️ **氧化的工作电位两种电极不同**（微针 `+0.4 V`、丝网印刷 `+0.2 V`），还原两者都是 `-0.2 V`；电极维度另外改电流量程；偏置一律取 `20% FSR`：

| 预设 | E | V_WE | RE | FSR | offset | 时长 |
| --- | ---: | ---: | ---: | ---: | ---: | ---: |
| `微针 +0.4V` | `+0.4 V` | `1.2 V` | `0.800 V` | `50 nA` | `10 nA` | `180 s` |
| `丝网印刷 +0.2V` | `+0.2 V` | `1.2 V` | `1.000 V` | `1 µA` | `200 nA` | `180 s` |
| `微针 -0.2V` | `-0.2 V` | `0.25 V` | `0.450 V` | `50 nA` | `10 nA` | `120 s` |
| `丝网印刷 -0.2V` | `-0.2 V` | `0.25 V` | `0.450 V` | `1 µA` | `200 nA` | `120 s` |

同一档的两个按钮必须至少差一个字段，否则会同时高亮（高亮只比设置值，不记住点了哪个）。量程差异同时决定分辨率：有效 LSB ≈ FSR/2^14 ⇒ 微针档约 `3 pA`、丝网印刷档约 `61 pA`；把小电流体系挂到 1 µA 档等于放弃 20× 分辨率，换量程前先确认信号确实需要它。`RE = V_WE − E` 必须落在 DAC 的 `0.008–1.535 V`，改电位时最容易踩的就是这条（`+0.4 V` 配 `V_WE 0.25 V` ⇒ `RE = −0.15 V`）；`test_every_it_quick_preset_passes_the_backend_validator` 会用后端校验器逐个过一遍。
