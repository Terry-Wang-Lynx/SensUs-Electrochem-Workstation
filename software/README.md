...
界面的快速预设按“两种电极 × 两个工作电位”排成四个。电极维度只改电流量程，电位维度只改 E、V_WE 与时长；偏置一律取 `20% FSR`：
...
两种电极必须至少差一个字段，否则同电位的两个按钮会同时高亮（高亮只比设置值，不记住点了哪个）。量程差异同时决定分辨率：有效 LSB ≈ FSR/2^14 ⇒ 微针档约 `3 pA`、丝网印刷档约 `61 pA`；把小电流体系挂到 1 µA 档等于放弃 20× 分辨率，换量程前先确认信号确实需要它。
