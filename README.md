...
浏览器模式在 macOS 上由当前用户的 `launchd` 服务托管，关闭启动脚本的终端窗口不会停止后端；后端异常退出时会自动拉起。网页右上角的“退出”是显式停止操作，日志位于 `~/Library/Logs/SensUs Workstation/`。

界面操作失败时会显示“诊断编号”；在“硬件 DEBUG”页底部的“应用诊断”可查看本次会话中的设备、参数应用、固件编译/烧录、测量和自动任务事件。“下载诊断包”会导出已脱敏的运行环境、轮转日志和当前轮次的采集器/硬件审计文件，不包含原始测量 CSV。Windows 默认日志位于 `%LOCALAPPDATA%\SensUs Workstation\logs\`。
